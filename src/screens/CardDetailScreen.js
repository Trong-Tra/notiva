...
export default function CardDetailScreen({ route, navigation }) {
  ...
  const deleteChecklist = (clId) => {
    setChecklists(checklists.filter((cl) => cl.id !== clId));
  };

  const addChecklistItem = (clId) => {
    const text = newItemInputs[clId]?.trim();
    if (!text) return;
    setChecklists(
      checklists.map((cl) =>
        cl.id === clId
          ? { ...cl, items: [...cl.items, { id: generateId(), title: text, isChecked: false }] }
          : cl
      )
    );
      ...
      <ScrollView style={styles.content} keyboardShouldPersistTaps="handled">
            ...
            {dueDate ? (
              <View style={styles.dateDisplay}>
                <Text style={styles.dateText}>
                  {formatDate(dueDate.getTime())} {hasTime ? formatTime(dueDate.getTime()) : ''}
                </Text>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => setLabelModalVisible(true)}>
          ...
          <View style={styles.labelList}>
            {labels.map((label) => (
              <View key={label.id} style={[styles.labelChip, { backgroundColor: label.color }]}>
                ...
                <TouchableOpacity onPress={() => removeLabel(label.id)}>
                  <Text style={styles.labelChipRemove}>×</Text>
                </TouchableOpacity>
              </View>
            ))}
            {labels.length === 0 && (
              <Text style={styles.placeholderText}>No labels</Text>
            )}
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => setChecklistModalVisible(true)}>
              <Text style={styles.addLink}>+ Add</Text>
            </TouchableOpacity>
          </View>
          {checklists.map((cl) => (
            <View key={cl.id} style={styles.checklistBox}>
              ...
              <View style={styles.addItemRow}>
                <TextInput
                  ...
                  onChangeText={(text) =>
                    setNewItemInputs({ ...newItemInputs, [cl.id]: text })
                  }
                  onSubmitEditing={() => addChecklistItem(cl.id)}
                />
                <TouchableOpacity onPress={() => addChecklistItem(cl.id)}>
                  <Text style={styles.addLink}>Add</Text>
                </TouchableOpacity>
              </View>
            </View>
          ...
          {checklists.length === 0 && (
            <Text style={styles.placeholderText}>No checklists</Text>
          )}
        </View>
              ...
              {labelColors.map((color) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    selectedLabelColor === color && styles.colorSelected,
                  ]}
                  onPress={() => setSelectedLabelColor(color)}
                />
              ))}
            </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <View style={styles.modalRow}>
              ...
              <TouchableOpacity style={styles.modalBtnPrimary} onPress={addChecklist}>
                <Text style={{ color: '#fff' }}>Add</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.modalBox}>
            ...
            {board?.lists.map((l) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  if (l.id !== listId) {
                    moveCard(boardId, listId, l.id, cardId, l.cards.length);
                    navigation.goBack();
                  }
